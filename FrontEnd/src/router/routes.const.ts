export const ROUTES = {
  HOME: "/",
  PRODUCTS: "/products",
  SERVICES: "/services",
  PRODUCT_DETAIL: "/products/:slug",
  CATEGORY: "/category/:slug",
  SEARCH: "/search",

  LOGIN: "/login",
  SIGNUP: "/signup",
  FORGOT_PASSWORD: "/forgot-password",

  CART: "/cart",
  CHECKOUT: "/checkout",
  ORDER_SUCCESS: "/order/success",
  ORDER_CANCEL: "/order/cancel",
  ORDER_HISTORY: "/orders",
  ORDER_DETAIL: "/orders/:orderId",
  PROFILE: "/profile",
  WISHLIST: "/wishlist",
  MEMBERSHIP: "/membership",

  ADMIN: "/admin",
  ADMIN_DASHBOARD: "/admin/dashboard",
  ADMIN_USERS: "/admin/users",
  ADMIN_USER_DETAIL: "/admin/users/:userId",
  ADMIN_EMPLOYEES: "/admin/employees",
  ADMIN_EMPLOYEE_DETAIL: "/admin/employees/:employeeId",
  ADMIN_PAYMENTS: "/admin/payments",
  ADMIN_REPORTS: "/admin/reports",
  ADMIN_PRODUCTS: "/admin/products",
  ADMIN_PRODUCT_CREATE: "/admin/products/create",
  ADMIN_PRODUCT_EDIT: "/admin/products/:id/edit",
  ADMIN_BRANDS: "/admin/brands",
  ADMIN_CATEGORIES: "/admin/categories",
  ADMIN_ORDERS: "/admin/orders",
  ADMIN_ORDER_DETAIL: "/admin/orders/:orderId",
  ADMIN_PROMOTIONS: "/admin/promotions",
  ADMIN_BLOGS: "/admin/blogs",
  ADMIN_BLOG_CREATE: "/admin/blogs/create",
  ADMIN_BLOG_EDIT: "/admin/blogs/:blogId/edit",
  ADMIN_BLOG_PREVIEW: "/admin/blogs/:blogId/preview",
  ADMIN_FEEDBACK: "/admin/feedback",
  ADMIN_PRODUCT_VERSIONS: "/admin/product-versions",
  ADMIN_AUDIT_LOGS: "/admin/audit-logs",

  STAFF: "/staff",
  STAFF_ORDERS: "/staff/orders",
  STAFF_ORDER_CREATE: "/staff/orders/create",
  STAFF_ORDER_CONFIRM: "/staff/orders/confirm",
  STAFF_ORDER_PAYMENT_RESULT: "/staff/orders/payment-result",
  STAFF_ORDER_DETAIL: "/staff/orders/:orderId",
  STAFF_PRODUCTS: "/staff/products",
  STAFF_PRODUCT_CREATE: "/staff/products/create",
  STAFF_PRODUCT_EDIT: "/staff/products/:id/edit",
  STAFF_BRANDS: "/staff/brands",
  STAFF_CATEGORIES: "/staff/categories",
  STAFF_PROMOTIONS: "/staff/promotions",
  STAFF_BLOGS: "/staff/blogs",
  STAFF_BLOG_CREATE: "/staff/blogs/create",
  STAFF_BLOG_EDIT: "/staff/blogs/:blogId/edit",
  STAFF_BLOG_PREVIEW: "/staff/blogs/:blogId/preview",
  STAFF_USERS: "/staff/users",
  STAFF_FEEDBACK: "/staff/feedback",
  STAFF_PRODUCT_VERSIONS: "/staff/product-versions",

  UNAUTHORIZED: "/401",
  FORBIDDEN: "/403",
  SERVER_ERROR: "/500",
  SERVICE_UNAVAILABLE: "/503",
  GATEWAY_TIMEOUT: "/504",
} as const;
